...
public class ShopCartPresenter extends BasePresenter<ShopCartFragment> {
    ...
    public void onCreate() {
        net.getShopCart(mView.getContext());
    }
    ...
    public void disposeInfoMsg(EventMessage message) {
        switch (message.getDataType()){
            case ShopCartAdaphter.POPUWINDOW:  //弹出商品规格页面
                ...
                popuWindow=new GoodsStandardPopuWindow(mView.getContext(),bean);
                popuWindow.showAtLocation(mView.rl_shopcart,Gravity.BOTTOM,0,0);
                popuWindow.setTouchable(true);
                popuWindow.setOutsideTouchable(true);
                WindowMagerUntil.backgroundAlpha(0.5f,mView.getContext());
                popuWindow.setOnDismissListener(new PopupWindow.OnDismissListener(){
                    @Override
                    public void onDismiss() {
                        WindowMagerUntil.backgroundAlpha(1.0f,mView.getContext());
                    }
                });
                break;
    ...
    public void submit(){
        switch (type){
            case TYPE_NORMAL:
                mView.startFragment(new ResultFragment());
                break;
            case TYPE_EDITER:
                ...
                net.getDelCart(sb.toString(),mView.getContext());
                break;
        }
    }
