    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch (ev.getActionMasked()) {
            case MotionEvent.ACTION_DOWN: {
                if (suDu == null)
                    suDu = VelocityTracker.obtain();//获得VelocityTracker类实例
                else
                    suDu.clear();
                mLastY = ev.getY();
                suDu.addMovement(ev);
            ...
            case MotionEvent.ACTION_MOVE: {
                suDu.addMovement(ev);//将事件加入到VelocityTracker类实例中
                ...
                mLastY = ev.getY();
                //	requestLayout：当view确定自身已经不再适合现有的区域时，该view本身调用这个方法
            ...
            case MotionEvent.ACTION_UP: {
                ...
                suDu.computeCurrentVelocity(700);
                // 获取Y方向的速度 可以通过getXVelocity()和getYVelocity()获得横向和竖向的速率
                float mYV = suDu.getYVelocity();
