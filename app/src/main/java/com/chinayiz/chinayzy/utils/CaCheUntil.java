...
public class CaCheUntil {
    private static CaCheUntil instance;
    ...
    public static CaCheUntil getInstance() {
        if (null == instance) {
            instance = new CaCheUntil();
        }
        return instance;
    }
    ...
    public  static String getCacheSize() {
        try {
            return getFormatSize(getFolderSize(new File(APP.getInstance().getCacheDir() + "/" +GLIDE_CARCH_DIR)));
        } catch (Exception e) {
            e.printStackTrace();
            return "获取失败";
        }
    ...
    public boolean cleanCatchDisk() {
        return deleteFolderFile(APP.getInstance().getCacheDir() + "/" + GLIDE_CARCH_DIR, true);
    }
